...
public class CourseController {
  ...
  public CourseDTO createCourse(@RequestParam("courseID") String courseID,
      @RequestParam("faculty") String faculty, @RequestParam("year") String year,
      @RequestParam("semester") String semester, @RequestParam("section") String section,
      @RequestParam("name") String courseName) {
    Course course = courseService.createCourse(courseID, faculty, semester.toUpperCase(), year,
        section, courseName);
  ...
  public CourseDTO unregisterStudent(@RequestParam("email") String email,
      @RequestParam("id") int id) {
    Account account = accountService.getAccountByID(email);
    Student student = (Student) account.getUserRole();
    Course course = courseService.getCourseByID(id);
  ...
  public List<CourseDTO> getEnrolledCourses(@PathVariable("studentId") String email) {
    ...
    for (Course course : courses) {
      CourseDTO courseDTO =
          new CourseDTO(course.getId(), course.getCourseID(), course.getFaculty(), course.getYear(),
              course.getSemester().toString(), course.getCourseSection(), course.getCourseName());
      courseDTOs.add(courseDTO);
    }
    return courseDTOs;
  }
  ...
  @GetMapping(
      value = {"courses/byYearAndSemesterAndSection", "courses/byYearAndSemesterAndSection/"})
  public List<CourseDTO> getCoursesByYearBySemesterBySection(@RequestParam("year") String year,
      @RequestParam("semester") String semester, @RequestParam("section") String section) {
    List<Course> courses =
        courseService.getCoursesByYearBySemesterBySection(year, semester, section);
    return DTOUtil.convertToCourseDTOs(courses);
  }
