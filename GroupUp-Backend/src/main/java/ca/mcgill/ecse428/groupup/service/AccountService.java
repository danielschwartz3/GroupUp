...
public class AccountService {
  ...
  @Transactional
  public Account createAdminAccount(Admin role, String userName, String name, String email,
      String institution, String password) {
    Account acc;
    String error = "";
    Boolean isEmailValid = false;
  ...
  public Account changeUserInformation(String oldEmail, String newUserName, String newName,
    ...
    oldAccount =
        setAccountDetails(oldAccount, newUserName, newName, newEmail, newInstitution, "hack");
  ...
  private String verifyInput(UserRole role, String userName, String name, String email,
      String institution, String password) {
    String error = "";
    if (role == null)
      error += "User's role cannot be empty";
    if (userName == null || userName.trim().length() == 0)
      error += "Username cannot be empty";
    if (name == null || name.trim().length() == 0)
      error += "User's full-name cannot be empty";
    if (email == null || email.trim().length() == 0)
      error += "User's email cannot be empty";
    if (institution == null || institution.trim().length() == 0)
      error += "User institution cannot be empty";
    if (password == null || password.trim().length() == 0)
      error += "Password cannot be empty";
    return error;
  }

  private Account setAccountDetails(Account acc, String userName, String name, String email,
      String institution, String password) {
    acc.setUserName(userName);
    acc.setName(name);
    acc.setEmail(email);
